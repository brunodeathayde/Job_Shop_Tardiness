...
function MILP_Liao_tardiness(O,P,d,V,M,N,time_limit)
    ...
    @variable(model, q[j in 1:N-1, k in j+1:N, i in 1:M] >=0)
    ...
    @constraint(model, [j in 1:N-1, k in j+1:N, i in 1:M],  V*z[j,k,i] + (x[j,i] - x[k,i]) - P[k,i] == q[j,k,i])
    @constraint(model, [j in 1:N-1, k in j+1:N, i in 1:M], q[j,k,i] <= V - P[j,i] - P[k,i])
